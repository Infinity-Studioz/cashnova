...
async function generateUserAlerts(userId: string): Promise<any[]> {
  ...
  try {
    ...
    const totalMonthlySpent = monthlySpending.reduce((sum, cat) => sum + cat.totalSpent, 0);
    ...
    if (alertSettings.categoryThreshold.enabled && currentBudget) {
      ...
      for (const categorySpending of monthlySpending) {
        ...
        if (categoryBudget && spent >= (categoryBudget.allocated * threshold)) {
          const percentage = Math.round((spent / categoryBudget.allocated) * 100);
          ...
          if (!recentAlert && percentage >= alertSettings.categoryThreshold.percentage) {
            const notification = Notification.createNigerianAlert(
              ...
              `Heads-up: You're at ${percentage}% of your ${categoryName.toLowerCase()} budget (₦${spent.toLocaleString()}/₦${categoryBudget.allocated.toLocaleString()}).`,
              {
                ...
                actions: [
                  { label: 'View Breakdown', action: 'view_category_breakdown', data: { category: categoryName } },
                  { label: 'Adjust Budget', action: 'adjust_budget', data: { category: categoryName } }
                ]
              },
              'medium'
            );
    ...
    if (alertSettings.budgetExceeded.enabled && currentBudget) {
      ...
      if (totalMonthlySpent > (budgetLimit * exceedThreshold)) {
        const exceedPercentage = Math.round(((totalMonthlySpent - budgetLimit) / budgetLimit) * 100);
        ...
        if (!recentAlert) {
          const notification = Notification.createNigerianAlert(
            ...
            `Warning: You've exceeded your overall budget by ${exceedPercentage}% this month.`,
            {
              ...
              actions: [
                { label: 'View Breakdown', action: 'view_budget_breakdown' },
                { label: 'Adjust Budget', action: 'adjust_overall_budget' }
              ]
            },
            'high'
          );
      ...
      if (dayOfMonth >= 25 && dayOfMonth <= 28) {
        ...
        if (!recentSalaryAlert) {
          const notification = Notification.createNigerianAlert(
            ...
            {
              actions: [
                { label: 'Plan Budget', action: 'create_next_month_budget' },
                { label: 'Review Goals', action: 'view_savings_goals' }
              ]
            },
            'medium'
          );
      ...
      if ([0, 8].includes(currentMonth) && currentDate.getDate() <= 15) {
        ...
        if (!recentSchoolAlert) {
          ...
          const notification = Notification.createNigerianAlert(
            ...
            {
              actions: [
                { label: 'Add School Fees Budget', action: 'add_school_fees_category' },
                { label: 'View Education Expenses', action: 'view_category', data: { category: 'School Fees' } }
              ]
            },
            'medium'
          );
    ...
    const topSpendingCategory = monthlySpending.length > 0 ? 
      monthlySpending.reduce((prev, current) => prev.totalSpent > current.totalSpent ? prev : current) : null;

    if (topSpendingCategory && topSpendingCategory.totalSpent > 20000) { // High spending category
      ...
      if (!recentTip) {
        ...
        const notification = Notification.createNigerianAlert(
          ...
          `You could save ₦${potentialSavings.toLocaleString()} this month by reducing ${categoryName.toLowerCase()} from ₦${spent.toLocaleString()} to ₦${newTarget.toLocaleString()}.`,
          {
            ...
            potentialSavings,
            actions: [
              { label: 'Apply Suggestion', action: 'adjust_category_budget', data: { category: categoryName, newAmount: newTarget } },
    ...
    for (const customAlert of alertSettings.customAlerts) {
      ...
      const categorySpending = monthlySpending.find(cat => cat._id === customAlert.category);
      ...
      if (customAlert.condition === 'spending_reaches') {
        const threshold = customAlert.isPercentage ? 
          (currentBudget?.categories?.find((cat: any) => cat.name === customAlert.category)?.allocated || 0) * (customAlert.threshold / 100) :
        ...
        if (!recentCustomAlert) {
          const notification = Notification.createNigerianAlert(
            ...
            alertMessage,
            {
              ...
              actions: [
                { label: 'View Details', action: 'view_category_details', data: { category: customAlert.category } },
                { label: 'Edit Alert', action: 'edit_custom_alert', data: { alertId: customAlert.id } }
              ]
            },
            'medium'
          );
