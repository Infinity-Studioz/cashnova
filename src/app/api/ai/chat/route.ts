...
export async function POST(request: NextRequest) {
  try {
    ...
    const personalizedContext = buildPersonalizedContext(userTransactions, userBudget, userGoals);
    ...
    if (conversationId) {
      ...
      conversation = await Conversation.createFinancialCoachSession(
        session.user.email,
        message,
        context
      );
...
Provide helpful, Nigerian-context financial advice. Be specific and actionable.`
      ...
      const fallbackResponse = generateFallbackResponse(message, personalizedContext);
...
// Helper functions
function buildPersonalizedContext(transactions: any[], budget: any, goals: any[]): string {
  ...
  if (transactions.length > 0) {
    ...
    context += `- Top spending categories: ${topCategories.map(([cat, amount]) => 
  ...
  if (goals.length > 0) {
    context += `- Active goals: ${goals.map(g => 
...
async function generateActionableInsights(
  ...
  if (userMessage.toLowerCase().includes('budget')) {
    try {
      const budgetInsight = await AIInsight.generateSpendingInsight(userId, {
        category: 'Budget',
        percentageOfBudget: 50, // Placeholder - calculate from actual data
  ...
  if (userMessage.toLowerCase().includes('save') || userMessage.toLowerCase().includes('goal')) {
    insights.push({
      ...
      message: 'Based on your spending patterns, consider setting up automatic savings',
      action: 'create_goal'
    });
  }
...
function generateFollowUpSuggestions(context: string, message: string): string[] {
  const suggestions = [];
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (conversationId) {
      ...
      const conversations = await Conversation.getUserActiveConversations(
        session.user.email,
        limit
      );
