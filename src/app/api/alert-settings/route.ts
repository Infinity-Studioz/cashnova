...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Add to your PUT endpoint validation
    if (categoryThreshold && categoryThreshold.specificCategory) {
      ...
      if (!validCategories.includes(categoryThreshold.specificCategory)) {
        return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { category, condition, threshold, isPercentage, notificationTiming } =
      body;
    ...
    if (!category || !condition || typeof threshold !== "number") {
      return NextResponse.json(
        {
          error:
            "Missing required fields: category, condition, and threshold are required.",
        },
        { status: 400 }
      );
    }
    ...
    if (
      !["spending_reaches", "daily_exceeds", "weekly_exceeds"].includes(
        condition
      )
    ) {
      return NextResponse.json(
        {
          error:
            "Invalid condition. Must be spending_reaches, daily_exceeds, or weekly_exceeds.",
        },
        { status: 400 }
      );
    }
