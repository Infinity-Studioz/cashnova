  ...
  try {
    ...
    if (isEndOfMonth && source === "manual") {
      insights.push({
        type: "timing",
        message:
          "Perfect timing! End-of-month contributions align with Nigerian salary cycles",
    ...
    if (!goal.autoSaveRules?.enabled && goal.contributions.length >= 3) {
      recommendations.push({
        type: "automation",
        message:
          "You're building a great saving habit! Consider enabling auto-save for consistency",
    ...
    if (isCompleted) {
      recommendations.push({
        type: "next_steps",
        message:
          "Goal completed! Consider setting a new goal to maintain your savings momentum",
      ...
      recommendations.push({
        type: "final_push",
        message:
          "You're in the final stretch! Consider a final push to complete this goal",
    ...
    const response = {
      ...
      achievements: {
        milestones: newAchievements.map((m: any) => ({
