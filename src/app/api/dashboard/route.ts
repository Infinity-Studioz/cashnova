  ...
  try {
    ...
    const spendingByCategory = categorySpending.map(cat => {
      ...
      return {
        ...
        budgetUtilization: categoryBudget ? (cat.amount / categoryBudget.allocated) * 100 : 0,
        nigerianInsights: generateCategoryInsights(cat._id, cat.amount, percentage)
      };
    });
    ...
    const weeklySpending = weeklyData.map((week, index) => ({
      ...
      variance: weeklyBudgetAmount > 0 ? ((week.spending - weeklyBudgetAmount) / weeklyBudgetAmount) * 100 : 0
    }));
    ...
    const formattedGoals = userGoals.map(goal => {
      ...
      return {
        ...
        monthlyContributionNeeded: goal.insights?.monthlyContributionNeeded || 0,
    ...
    const dashboardData = {
      overview: {
        ...
        healthScore: calculateFinancialHealthScore(totalBalance, currentIncome, currentExpenses)
      },
      spendingByCategory,
      weeklySpending,
...
function calculateInflationImpact(currentExpenses: number, prevExpenses: number): number {
...
function calculateFinancialHealthScore(balance: number, income: number, expenses: number): number {
