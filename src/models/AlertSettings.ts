...
// Add before: export default mongoose.models.AlertSettings || mongoose.model(...)
AlertSettingsSchema.statics.getDefaultSettings = function (userId: string) {
  return {
    userId,
    categoryThreshold: {
      enabled: true,
    ...
    budgetExceeded: { enabled: true, percentage: 5 },
    weeklySummary: { enabled: false, day: "Sunday", time: "9:00 AM" },
    customAlerts: [], // Critical: must be an array, not undefined
    notificationPreferences: {
      pushNotifications: true,
      emailAlerts: true,
      smsAlerts: false,
...
export default mongoose.models.AlertSettings ||
  mongoose.model<IAlertSettings, IAlertSettingsModel>(
    "AlertSettings",
    AlertSettingsSchema
  );
